...
aapl %>%
  select(-date,-ticker,-pos_neg, -open, -close, -high, -low, -lead_adj_close) %>%
  cor %>%
  ggcorrplot
```
...
ggplot(data = aapl %>% slice(1:60)) + 
  geom_point(aes(x = date, y = macd)) + 
  geom_point(aes(x = date, y = macd_signal, color = 'blue'))
